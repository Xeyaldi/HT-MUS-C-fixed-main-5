...
def build_stream(path, video: bool = False, ffmpeg_parameters: str = None):
    """
    Build a pytgcalls 2.x MediaStream, replacing the old
    AudioPiped / AudioVideoPiped (+ HighQualityAudio / MediumQualityVideo) objects.
    """
    kwargs = {}
    if ffmpeg_parameters:
        kwargs["ffmpeg_parameters"] = ffmpeg_parameters
    if video:
        return MediaStream(
            path,
...
class Call(PyTgCalls):
    ...
    async def stream_call(self, link):
        ...
        await assistant.play(config.LOGGER_ID, build_stream(link, video=True))
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if "live_" in queued:
                ...
                stream = build_stream(link, video=video)
                try:
                ...
                stream = build_stream(file_path, video=video)
                try:
                    await client.play(chat_id, stream)
                except:
                    return await app.send_message(
                ...
                stream = build_stream(videoid, video=str(streamtype) == "video")
                try:
                    await client.play(chat_id, stream)
                except:
                    return await app.send_message(
                ...
                stream = build_stream(queued, video=bool(video))
                try:
                    await client.play(chat_id, stream)
                except:
                    return await app.send_message(
